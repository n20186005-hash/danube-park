...
export default function DanubeParkIntro() {
  const t = useTranslations('danubePark.intro');
  const tOff = useTranslations('danubePark.officialManagement');
  const messages = useMessages() as any;
  const items: string[] = messages?.danubePark?.intro?.visitGuide?.items || [];
  const alsoKnownAsItems: string[] = messages?.danubePark?.intro?.alsoKnownAs?.items || [];
